/**
 * App.jsx — TourNet root
 *
 * Routing architecture (no react-router needed):
 *
 *   AuthProvider  ← manages Supabase session globally
 *     └─ AppGate
 *          ├─ OAuthCallbackScreen   ← shown while processing Google redirect
 *          └─ ProtectedRoute
 *               ├─ authenticated  → <Dashboard />
 *               └─ unauthenticated → <UnauthenticatedShell />
 *
 * Google OAuth flow:
 *   1. User clicks "Continue with Google"
 *   2. Browser → Google consent → Supabase callback → App (/auth/callback?code=)
 *   3. `isOAuthCallback()` detects the code in the URL
 *   4. OAuthCallbackScreen renders while supabase-js exchanges code → session
 *   5. onAuthStateChange fires → AppGate renders Dashboard
 *   6. URL is cleaned (no ?code= visible to user)
 */
...
/* ═══════════════════════════════════════════════════════════════════
   GOOGLE OAUTH CALLBACK SCREEN
   Shown while supabase-js exchanges the Google code for a session.
   Replaces the generic loading spinner with a Google-branded screen.
═══════════════════════════════════════════════════════════════════ */
const GoogleSVG = () => (
  ...
  return (
    ...
    >
      ...
      <div style={{ textAlign: 'center', zIndex: 1 }}>
        ...
        <p style={{ margin: 0, fontSize: 13, color: 'rgba(190,165,140,0.65)', lineHeight: 1.5 }}>
          Verifying your account and setting up your profile…
        </p>
      </div>
...
function UnauthenticatedShell() {
  ...
  return (
    <>
      <AuthPage key={initialView} initialView={initialView} />
      <LoginPromptModal
        onLogin={() => setInitialView('login')}
        onSignup={() => setInitialView('signup')}
      />
    </>
  );
}
...
function AppGate() {
  ...
  return (
    <ProtectedRoute fallback={<UnauthenticatedShell />}>
      <Dashboard />
    </ProtectedRoute>
  );
}
